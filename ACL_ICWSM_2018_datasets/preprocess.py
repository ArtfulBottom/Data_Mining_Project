...
df = pd.read_csv('./ACL_ICWSM_2018_datasets/nepal/2015_Nepal_Earthquake_train.tsv', sep='\t', encoding='iso-8859-1')
print(df)
...
df = df.apply(lambda x: json.dumps({'tweet_id': str(x['tweet_id']), 'text': x['text'], 'label': x['label']}), axis=1)
# print(df.head())
df.to_csv('./data/ids.json', header=False, index=False, quoting=csv.QUOTE_NONE, sep='|', escapechar='|')
