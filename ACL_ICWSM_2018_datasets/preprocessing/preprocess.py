...
df_train = pd.read_csv('./ACL_ICWSM_2018_datasets/nepal/2015_Nepal_Earthquake_train.tsv', sep='\t', encoding='iso-8859-1')
df_train = df_train.apply(lambda x: json.dumps({'tweet_id': str(x['tweet_id']), 'text': x['text'], 'label': x['label']}), axis=1)

df_dev = pd.read_csv('./ACL_ICWSM_2018_datasets/nepal/2015_Nepal_Earthquake_dev.tsv', sep='\t', encoding='iso-8859-1')
df_dev = df_dev.apply(lambda x: json.dumps({'tweet_id': str(x['tweet_id']), 'text': x['text'], 'label': x['label']}), axis=1)

df_test = pd.read_csv('./ACL_ICWSM_2018_datasets/nepal/2015_Nepal_Earthquake_test.tsv', sep='\t', encoding='iso-8859-1')
